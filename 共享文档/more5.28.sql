...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_add`(IN `contents` TEXT)
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_detail`(IN `id` INT,IN `witch` TEXT)
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_list`(IN `filterinfo` TEXT, IN `orderinfo` TEXT, IN `page` INT, IN `length` INT)
    NO SQL
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_stat`()
BEGIN
	#Routine body goes here...
  SELECT COUNT(*) INTO @jinrihuiyuan  FROM `user`.user_basic WHERE `user`.user_basic.registertime >=(NOW() - interval 24 hour);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_update`(IN `id` INT, IN `contents` TEXT,IN `witch` TEXT)
    NO SQL
BEGIN
