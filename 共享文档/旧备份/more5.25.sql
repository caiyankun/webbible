...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_add`(IN `p_uname` VARCHAR(100), IN `p_upass` VARCHAR(100))
    NO SQL
BEGIN
  declare userexist int default 0;
  call `user`.username_check(p_uname,userexist);
  if userexist>0 then
       SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'User already exist!';
  else
       insert into `user`.user_basic set uname=p_uname,upass=p_upass,user_basic.ulevel=101,lastlogintime=CURRENT_TIMESTAMP;
       if row_count()<1 THEN
       		SIGNAL SQLSTATE '45001' SET MESSAGE_TEXT = 'Insert failed! Affected row is 0';
       end if;
  end if;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_detail`(IN `uid` INT)
    NO SQL
BEGIN
...
SELECT (page-1)*length AS st,length,@totalqty;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `user_update`(IN `uid` INT, OUT `quota` INT)
    NO SQL
BEGIN
