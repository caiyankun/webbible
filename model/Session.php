...
class Session
{
    ...
    public static function boot()
    {
     ...
     /**
     * session检查，检查给定的值是否是Session中存储的值，
     * @param string        $name session名称
     * @param mixed         $value session值
     * @param string|null   $prefix 作用域（前缀）
     * $delete               检查完后是否删除Session中的值
     * @return void
     */
    public static function check($name, $value = '',$prefix = null, $delete = false)
    {
        empty(self::$init) && self::boot();
        $result = self::get($name, $prefix);
        if ($result) {
            if ($delete) {self::delete($name, $prefix);}
            return ($result==$value); 
        } else {
            return false;
        }
